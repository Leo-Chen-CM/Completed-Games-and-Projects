...
Game::Game(sf::ContextSettings settings) : 
	window(VideoMode(800, 600), 
	"Bootleg Obstacle Avoidance Simulator", 
	sf::Style::Default, 
	settings)
{
	thePlayer = new Player;
	for (int i = 0; i < MAXIMUMCUBES; i++)
	{
...
void Game::initialize()
{
	...
	//Music
	if (!m_gameMusic.openFromFile("Assets/Music/Laserpack.ogg"))
	{
	...
	playerImgData = stbi_load(playerFileName.c_str(), &width, &height, &comp_count, 4);

	if (playerImgData == NULL)
	{
		DEBUG_MSG("ERROR: Texture not loaded");
	}
	...
	// Camera Matrix
	view = lookAt(
		vec3(xAxis, 4.0f, 15.0f),	// Camera (x,y,z), in World Space
		vec3(xAxis, 0.0f, 0.0f),		// Camera looking at origin
...
void Game::update()
{
	...
	if (gameActive == true)
	{
		for (int i = 0; i < MAXIMUMCUBES; i++)
		{
			if (collisionDetection(thePlayer->getPlayerObject(), theEnemies[i]->getEnemyObject()))
			{
			...
			theEnemies[i]->updateNPC(thePlayer->getPlayerPosition().x);
		}
		processButtons();
		thePlayer->move();
...
void Game::render()
{
	...
	for (int i = 0; i < MAXIMUMCUBES; i++)
	{
		theEnemies[i]->draw(x_offsetID, y_offsetID, z_offsetID);
	}

	thePlayer->draw(x_offsetID, y_offsetID, z_offsetID);
...
void Game::unload()
{
	...
	stbi_image_free(playerImgData);		// Free image stbi_image_free(..)
}
...
{
	std::cout << thePlayer->getPlayerPosition().x << ", "<< thePlayer->getPlayerPosition().y << ", " << thePlayer->getPlayerPosition().z << std::endl;
}
...
bool Game::collisionDetection(GameObject& one, GameObject& two)
{
	...
	// collision x-axis
	bool collisionX = one.getPosition().x + 2.0f >= two.getPosition().x &&
		two.getPosition().x + 2.0f >= one.getPosition().x;
	// collision z-axis
	bool collisionY = one.getPosition().z + 2.0f >= two.getPosition().z &&
		two.getPosition().z + 2.0f >= one.getPosition().z;

	// collision only if on both axes
	return collisionX && collisionY;
}

void Game::distanceTracker()
{
	...
	distanceString = "Distance: "
		+ string(toString(distance));

	distanceText.setString(distanceString);
	distanceText.setFont(font);

	distanceText.setFillColor(sf::Color(255, 255, 255, 170));
	distanceText.setPosition(18.f, 50.f);
